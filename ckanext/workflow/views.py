...
def reject(id):
    ...
    tk.get_action("move_to_previous_stage")(context, {"id": id, "reason": reason})
    endpoint = request.referrer or "/"
    return tk.redirect_to(endpoint)
...
def create_revision(id):
    ...
    pkg = tk.get_action("create_dataset_revision")(
        context,
        {
            "id": id,
        },
...
def pending_list():
    ...
    extra_vars = {
        "datasets": model.Session.query(model.Package).filter(
            model.Package.extras[wh._workflow_stage_field()].astext == "pending"
        )
    }
...
workflow.add_url_rule(
    "/workflow/dataset/<id>/approve", "workflow_approve", view_func=approve
)
workflow.add_url_rule(
...
workflow.add_url_rule(
    "/workflow/dataset/<id>/rescind", "workflow_rescind", view_func=rescind
)
workflow.add_url_rule(
    "/workflow/approvals", endpoint="workflow_pending_list", view_func=pending_list
)
workflow.add_url_rule(
    "/workflow/dataset/<id>/merge_revision",
    "merge_dataset_revision",
    view_func=merge_revision,
)
workflow.add_url_rule(
    "/workflow/dataset/<id>/create_revision",
    "create_dataset_revision",
    view_func=create_revision,
)
workflow.add_url_rule(
    "/workflow/dataset/<id>/purge", "purge_unpublished_dataset", view_func=purge
)
